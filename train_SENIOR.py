...
class Workspace(object):
    def __init__(self, cfg):
        ...
        self.update_curiosity_fre = self.cfg.curiosity_fre
        ...
        self.replay_buffer = ReplayBuffer(
            ...
            self.device,
            self.cfg.time_decrease,
            pre_buffer_capacity=int(self.cfg.max_feedback*self.cfg.segment),
        )
    ...
    def learn_reward(self, first_flag=0):
        ...
        if first_flag == 1:
            ...
            labeled_queries = self.reward_model.mds_sampling()
    ...
    def r_hat_critic_old(self, x):
        batch_size, segment_length, obsact = x.shape
    ...
    def bilevel_update(self):
        ...
        obs, action, reward, next_obs, not_done, not_done_no_max = self.replay_buffer.sample(self.agent.batch_size, self.step)
        ...
        if self.step % self.agent.critic_target_update_frequency == 0:
    ...
    def run(self):
        ...
        interact_count = 0
        start_time, episode_start_time = time.time(), time.time()
        while self.step < self.cfg.num_train_steps:
            ...
            if done:
                ...
                self.logger.log('train/total_duration', time.time()-start_time, self.step)
                self.logger.log('train/duration', time.time()-episode_start_time, self.step)
            ...
            if self.step == (self.cfg.num_seed_steps + self.cfg.num_unsup_steps):
                ...
                # update explore buffer
                self.replay_buffer.update_curiosity_buffer(self.step)  
                ...
                if self.total_feedback < self.cfg.max_feedback:
                    if interact_count == self.cfg.num_interact:
                        # update schedule / change mb_size
                        if self.cfg.reward_schedule == 1:
                            frac = (self.cfg.num_train_steps - self.step) / self.cfg.num_train_steps
                            if frac == 0:
                                frac = 0.01
                ...
                if self.step % self.cfg.num_meta_steps == 0 and self.total_feedback < self.cfg.max_feedback:  # 10000
                    self.bilevel_update()
                    self.replay_buffer.relabel_with_predictor(self.reward_model)
                else:
                    if self.step % self.update_curiosity_fre == 0:
                        # update explore buffer
                        self.replay_buffer.update_curiosity_buffer(self.step)  
...
@hydra.main(config_path='config/train_SENIOR.yaml', strict=True)
def main(cfg):
